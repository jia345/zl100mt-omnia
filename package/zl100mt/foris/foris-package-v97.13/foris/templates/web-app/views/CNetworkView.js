...
CNetworkView.prototype.render = function(){
    ...
    case 'netvTabGnss':
        //GNSS
	console.log('netvTabGnss:')
	console.log(oStore.store)
	console.log(oStore.store.GNSS.targetSim)
        let simStr = oStore.store.GNSS.targetSim
        $('#netvTabGnssTargetSimVal').val(simStr);
    break;
    case 'netvTabLan':
        // fill LAN
        if((undefined != oStore.store.LAN.LAN) && (0 != oStore.store.LAN.LAN.length)){
            //1.清除表格内容
        ...
        //DHCP
        $('#netvTabLanSIPVal').val(oStore.store.DHCP.startIP);
        $('#netvTabLanEIPVal').val(oStore.store.DHCP.endIP);
        $('#netvTabLanLeaseVal').val(oStore.store.DHCP.leaseTerm);
        $('#netvTabLanDefaultGWVal').val(oStore.store.DHCP.defaultGwIP);
        $('#netvTabLanDhcpDNS1Val').val(oStore.store.DHCP.DNS1);
        $('#netvTabLanDhcpDNS2Val').val(oStore.store.DHCP.DNS2);
        $("#netvTabLanDhcpSubmaskSelector").find("option:contains('" + oStore.store.DHCP.subMask + "')").attr("selected",true);
        $("#netvTabLan input[name=netvTabLanDHCPRadio][value='" + oStore.store.DHCP.dhcpStatus + "']").attr('checked','checked');
    ...
    case 'netvTabRTMP':
        ...
        oStore.store.LAN.LAN.forEach(function(value,index){
            ...
            if(oStore.store.RTMP.ServerIP == value.IP){ 
                $("#"+"RTMPInforIPValSelector"+ index).attr("selected",true);
                rtmpUrl = rtmpUrl + oStore.store.RTMP.ServerIP + ":1935/live/";
            }
        });
        ...
        $('#RTMPInforIPValSelector').change(()=>{
            ...
            rtmpUrl = "rtmp://" + strIP + ":1935/live/";
            $('#RTMPInforAddrVal').text(rtmpUrl);
            $('#RTMPInforAddrVal').css({"color": "red"});
        });
        //2. init the channel list table
        if((undefined != oStore.store.RTMP.channelList) && (0 != oStore.store.RTMP.channelList.length)){
            ...
            oStore.store.RTMP.channelList.forEach(function(value,index){
                let rtmpRefUrl = "rtmp://" + oStore.store.RTMP.ServerIP + ":1935/live/";
    ...
    break;
    case 'netvTabVpn':
        $('#netvTabVpnAddrVal').val(oStore.store.VPN.vpnAddress);
        $('#netvTabVpnNameVal').val(oStore.store.VPN.vpnUser);
        $('#netvTabVpnPwdVal').val(oStore.store.VPN.vpnPwd);
        $('#netvTabVpnKeyVal').val(oStore.store.VPN.vpnKey);
        $('#netvTabVpnStatusVal').text(oStore.store.VPN.vpnStatus);
        $("#netvTabVpnProtocolSelector").find("option:contains('" + oStore.store.VPN.vpnProtocol + "')").attr("selected",true);
    break;
    case 'netvTabRouting':
    ...
    case 'netvTabFWFilter':
        ...
        ('on' == oStore.store.FireWall.ipFilter)? ipFilter = true : ipFilter = false;
        ('on' == oStore.store.FireWall.macFilter)? macFilter = true : macFilter = false;
        ('on' == oStore.store.FireWall.DMZ.Status)? DMZ = true : DMZ = false;
        //
        ...
        // IP 过滤表
        if((undefined != oStore.store.FireWall.ipList) && (0 != oStore.store.FireWall.ipList.length)){
            $("#firewallIPFilterTbl tr:gt(1)").empty("");
            //"ipList":[{"Validation":"11", "LanIPs":"127.1.1.2", "LanPort":"3122","WLanIPs":"10.1.1.2", "WLanPort":"213", "Protocol":"SNMP", "Status":"enable"},
            // {"Validation":"12", "LanIPs":"127.1.1.3", "LanPort":"3123","WLanIPs":"10.1.1.3", "WLanPort":"214", "Protocol":"UDP", "Status":"diabale"}],
            // ** html 格式
            // <tr> <td><input type='checkbox' checked='' /></td> <td><input class='nvtpfValiVal'/></td> <td><input class='nvtpfLanIPVal'/></td> <td><input class='nvtpfLanPortVal'/></td>
            ...
            oStore.store.FireWall.ipList.forEach(function(value,index){
        ...
        // MAC 过滤表
        if((undefined != oStore.store.FireWall.macList) && (0 != oStore.store.FireWall.macList.length)){
            ...
            // <td><input class='nvTMFMemoVal' value=''/>描述</td> </tr>
                oStore.store.FireWall.macList.forEach(function(value,index){
    ...
    case 'netvTabMapping':
        // 端口映射
	console.log('netvTabMapping:')
	console.log(oStore.store.Mapping.portMapping)
        if((undefined != oStore.store.Mapping.portMapping) && (0 != oStore.store.Mapping.portMapping.length)){
            ...
            oStore.store.Mapping.portMapping.forEach(function(value,index){
                let wlanOptionStr = '';
                ...
                let trIndex = index;
                // insert LAN:IP list
                oStore.store.LAN.accessList.forEach((element, index)=>{ // create lan:ip select->options
        ...
        // console.log(oStore.store.Mapping)
        //--slotLTEZ
        // if((('on' == oStore.store.Mapping.slotLTEZ.LAN1)&&(true == this.checkSlotMappingRule('LAN1', 'LTE-Z')))
        if(('on' == oStore.store.Mapping.slotLTEZ.LAN1)
            ||(('on' != oStore.store.Mapping.slotLTEZ.LAN1)&&(false == this.checkSlotMappingRule('LAN1', 'LTE-Z')))){
            //
            changedFlg = false;
        }
        else{
            changedFlg = true;
            if('on' == oStore.store.Mapping.slotLTEZ.LAN1){
                oStore.store.Mapping.slotLTEZ.LAN1 = 'off';
            }
            else{
                oStore.store.Mapping.slotLTEZ.LAN1 = 'on';
            }
        }
        ('on' == oStore.store.Mapping.slotLTEZ.LAN1)? lan1Z = true : lan1Z = false;
        //
        //if((('on' == oStore.store.Mapping.slotLTEZ.LAN2)&&(true == this.checkSlotMappingRule('LAN2', 'LTE-Z')))
        if(('on' == oStore.store.Mapping.slotLTEZ.LAN2)
            ||(('on' != oStore.store.Mapping.slotLTEZ.LAN2)&&(false == this.checkSlotMappingRule('LAN2', 'LTE-Z')))){
            //
            changedFlg = false;
        }
        else{
            changedFlg = true;
            if('on' == oStore.store.Mapping.slotLTEZ.LAN2){
                oStore.store.Mapping.slotLTEZ.LAN2 = 'off';
            }
            else{
                oStore.store.Mapping.slotLTEZ.LAN2 = 'on';
            }
        }
        ('on' == oStore.store.Mapping.slotLTEZ.LAN2)? lan2Z = true : lan2Z = false;
        //
        // if((('on' == oStore.store.Mapping.slotLTEZ.LAN3)&&(true == this.checkSlotMappingRule('LAN3', 'LTE-Z')))
        if(('on' == oStore.store.Mapping.slotLTEZ.LAN3)
            ||(('on' != oStore.store.Mapping.slotLTEZ.LAN3)&&(false == this.checkSlotMappingRule('LAN3', 'LTE-Z')))){
            //
            changedFlg = false;
        }
        else{
            changedFlg = true;
            if('on' == oStore.store.Mapping.slotLTEZ.LAN3){
                oStore.store.Mapping.slotLTEZ.LAN3 = 'off';
            }
            else{
                oStore.store.Mapping.slotLTEZ.LAN3 = 'on';
            }
        }
        ('on' == oStore.store.Mapping.slotLTEZ.LAN3)? lan3Z = true : lan3Z = false;

        // console.log(lan1Z, lan2Z, lan3Z)
        //--slotLTE4G
        // if((('on' == oStore.store.Mapping.slotLTE4G.LAN1)&&(true == this.checkSlotMappingRule('LAN1', 'LTE-4G')))
        if(('on' == oStore.store.Mapping.slotLTE4G.LAN1)
            ||(('on' != oStore.store.Mapping.slotLTE4G.LAN1)&&(false == this.checkSlotMappingRule('LAN1', 'LTE-4G')))){
            //
            changedFlg = false;
        }
        else{
            changedFlg = true;
            if('on' == oStore.store.Mapping.slotLTE4G.LAN1){
                oStore.store.Mapping.slotLTE4G.LAN1 = 'off';
            }
            else{
                oStore.store.Mapping.slotLTE4G.LAN1 = 'on';
            }
        }
        ('on' == oStore.store.Mapping.slotLTE4G.LAN1)? lan14G = true : lan14G = false;
        //
        // if((('on' == oStore.store.Mapping.slotLTE4G.LAN2)&&(true == this.checkSlotMappingRule('LAN2', 'LTE-4G')))
        if(('on' == oStore.store.Mapping.slotLTE4G.LAN2)
            ||(('on' != oStore.store.Mapping.slotLTE4G.LAN2)&&(false == this.checkSlotMappingRule('LAN2', 'LTE-4G')))){
            changedFlg = false;
        }
        else{
            changedFlg = true;
            if('on' == oStore.store.Mapping.slotLTE4G.LAN2){
                oStore.store.Mapping.slotLTE4G.LAN2 = 'off';
            }
            else{
                oStore.store.Mapping.slotLTE4G.LAN2 = 'on';
            }
        }
        ('on' == oStore.store.Mapping.slotLTE4G.LAN2)? lan24G = true : lan24G = false;
        //
        // if((('on' == oStore.store.Mapping.slotLTE4G.LAN3)&&(true == this.checkSlotMappingRule('LAN3', 'LTE-4G')))
        if(('on' == oStore.store.Mapping.slotLTE4G.LAN3)
            ||(('on' != oStore.store.Mapping.slotLTE4G.LAN3)&&(false == this.checkSlotMappingRule('LAN3', 'LTE-4G')))){
            changedFlg = false;
        }
        else{
            changedFlg = true;
            if('on' == oStore.store.Mapping.slotLTE4G.LAN3){
                oStore.store.Mapping.slotLTE4G.LAN3 = 'off';
            }
            else{
                oStore.store.Mapping.slotLTE4G.LAN3 = 'on';
            }
        }
        ('on' == oStore.store.Mapping.slotLTE4G.LAN3)? lan34G = true : lan34G = false;
        ...
        if(true == changedFlg){
            ...
            //   }
            var selectedData = {
                "slotLTEZ" :oStore.store.Mapping.slotLTEZ,
                "slotLTE4G":oStore.store.Mapping.slotLTE4G
            };
            var str = {
            ...
            $.ajax({
                ...
                success: (res)=>{
                    ...
                    console.log(data);
                    if(0 == data.rc){
                        oStore.store.Mapping.slotLTEZ = selectedData.slotLTEZ;
                        oStore.store.Mapping.slotLTE4G = selectedData.slotLTE4G;
                    }
                    else{
        ...
        // IP/MAC绑定
	console.log("netvTabMappingMacIPTbl")
	console.log(oStore.store.Mapping.mac2ip)
        if((undefined != oStore.store.Mapping.mac2ip) && (0 != oStore.store.Mapping.mac2ip.length)){
            ...
            oStore.store.Mapping.mac2ip.forEach(function(value,index){
...
CNetworkView.prototype.checkSlotMappingRule = function(aLan, aWlan){
    ...
    for(let i = 0; i < oStore.store.Mapping.portMapping.length; i++){
        // console.log(oStore.store.Mapping.portMapping[i].WLanSlot+'<>'+oStore.store.Mapping.portMapping[i].LanSlot);
        if((oStore.store.Mapping.portMapping[i].WLanSlot == aWlan)&&(oStore.store.Mapping.portMapping[i].LanSlot == aLan)){
            // console.log(oStore.store.Mapping.portMapping[i].WLanSlot+'<>'+oStore.store.Mapping.portMapping[i].LanSlot);
            rc = true; //有映射关系
            break;
        }
    ...
    this.jqNode.load('views/CNetworkView.html', ()=>{
        ...
        //   }
            ...
            $.ajax({
                ...
                success: (res)=>{
                    ...
                    var data = res
                    if(0 == data.rc){
                        oStore.store.GNSS.targetSim = selectedData.targetSim;
        ...
        $('#netvTabLanApplyLanCfgBtn').click(()=>{
            ...
            $.ajax({
                ...
                success: (res)=>{
                    ...
                    var data = res
                    if(0 == data.rc){
                        oStore.store.LAN.LAN = selectedData;
                        tools.msgBox(tools.jsSwitchLang(enJsMap, cnJsMap, 'operationSucc')+ ' code:' + data.errCode); //'操作成功! 
                        console.log(data.dat);
                    }
                    else{
        ...
        $('#netvTabLanApplyDhcpBtn').click(()=>{
            ...
            var selectedData =  {
                ...
                "startIP": $('#netvTabLanSIPVal').val(),
                "endIP": $('#netvTabLanEIPVal').val(),
                "leaseTerm": $('#netvTabLanLeaseVal').val(),
                "subMask": $("#netvTabLanDhcpSubmaskSelector").children('option:selected').val(),
                "defaultGwIP": $('#netvTabLanDefaultGWVal').val(),
                "DNS1": $('#netvTabLanDhcpDNS1Val').val(),
                "DNS2": $('#netvTabLanDhcpDNS2Val').val()
            }
            var str = {
                "command":"setDhcpCfg",
                "dat":{
            ...
            $.ajax({
                ...
                success: (res)=>{
                    ...
                    var data = res
                    if(0 == data.rc){
                        oStore.store.DHCP = selectedData;
                        console.log(data.dat);
                        tools.msgBox(tools.jsSwitchLang(enJsMap, cnJsMap, 'operationSucc')+ ' code:' + data.errCode); //'操作成功! 
                    }
                    else{
        ...
        //   }
            ...
            $.ajax({
                ...
                success: (res)=>{
                    ...
                    if(0 == data.rc){
                        console.log(data.dat);
                        oStore.store.RTMP.ServerIP = selectedData;
        ...
        //   }
            ...
            $.ajax({
                ...
                success: (res)=>{
                    ...
                    var data = res
                    if(0 == data.rc){
                        oStore.store.RTMP.channelList = selectedData;
                        tools.msgBox(tools.jsSwitchLang(enJsMap, cnJsMap, 'operationSucc')+ ' code:' + data.errCode); //'操作成功! 
                        console.log(data.dat);
                    }
                    else{
        ...
        $('#netvTabVpnConnectBtn').click(()=>{
            ...
            $.ajax({
                ...
                success: (res)=>{
                    ...
                    var data = res
                    if(0 == data.rc){
                        oStore.store.VPN = selectedData;
                        oStore.store.VPN.vpnStatus = data.dat.VPN.vpnStatus;
                        $('#netvTabVpnStatusVal').text(oStore.store.VPN.vpnStatus);
                        tools.msgBox(tools.jsSwitchLang(enJsMap, cnJsMap, 'operationSucc')+ ' code:' + data.errCode); //'操作成功! 
                    }
                    else{
        ...
        function routingOperation(action){
            ...
            if((undefined == dstNet)||(undefined == subMask)||(undefined == gwIP)||(undefined == ifName)||(undefined == Metric)
                ||('' == dstNet)||('' == subMask)||('' == gwIP)||('' == ifName)||('' == Metric)){
                // alert("WLAN Port不能为空!");
            ...
            var str = {
                ...
                "dat":{
                    "operation": action,
                    "routingData": {
                        "dstNet": $('#netvTabRoutingInputDstNet').val(),
                        "subMask": $('#netvTabRoutingInputSubmask').val(),
                        "gwIP": $('#netvTabRoutingInputGW').val(),
                        "ifName": $('#netvTabRoutingInputIf').val(),
                        "Metric": $('#netvTabRoutingInputMetric').val()
                    }
                }
            }
            parameters = JSON.stringify(str);
        ...
        $('#nvtMApplyBtn').click(()=>{
            ...
            // });
            selectedData.DMZ.IP = $('#nvtMDMZIPAddressVal').val();
            // "dat": {
            //     "FireWall":{
            ...
            $.ajax({
                ...
                success: (res)=>{
                    ...
                    var data = res
                    if(0 == data.rc){
                        oStore.store.FireWall.ipFilter = selectedData.ipFilter;
                        oStore.store.FireWall.macFilter = selectedData.macFilter;
                        oStore.store.FireWall.DMZ = selectedData.DMZ;
                        tools.msgBox(tools.jsSwitchLang(enJsMap, cnJsMap, 'operationSucc')+ ' code:' + data.errCode); //'操作成功! 
                        console.log(data.dat);
                    }
                    else{
        ...
        $('#nvTPFWIPFilterApplyallBtn').click(()=>{
            ...
            $("#firewallIPFilterTbl").find("tr").each(function(idx, item){
                ...
                if((undefined != Validation) && (undefined != LanIPs) && (undefined != LanPort) && (undefined != WLanIPs) && (undefined != WLanPort) && (undefined != Protocol) && (undefined != Status)){
                    if(('' == Validation) || ('' == LanIPs) || ('' == LanPort) || ('' == WLanIPs) || ('' == WLanPort) || ('' == Protocol) || ('' == Status)){
                        // alert("WLAN Port不能为空!");
            ...
            // "dat": {
            //     "FireWall":{
            //       "ipList":[{"Validation":"xx", "LanIPs":"xx", "LanPort":"xx",
            ...
            $.ajax({
                ...
                success: (res)=>{
                    ...
                    var data = res
                    if(0 == data.rc){
                        oStore.store.FireWall.ipList = selectedData;
                        tools.msgBox(tools.jsSwitchLang(enJsMap, cnJsMap, 'operationSucc')+ ' code:' + data.errCode); //'操作成功! 
                        console.log(data.dat);
                    }
                    else{
        ...
        $('#nvTMFMacFilterApplyallBtn').click(()=>{
            ...
            $.ajax({
                ...
                success: (res)=>{
                    ...
                    var data = res
                    if(0 == data.rc){
                        oStore.store.FireWall.macList = selectedData;
                        tools.msgBox(tools.jsSwitchLang(enJsMap, cnJsMap, 'operationSucc')+ ' code:' + data.errCode); //'操作成功! 
                        console.log(data.dat);
                    }
                    else{
        ...
        $('#netvTabMappingPortTblNewBtn').click(()=>{
            ...
            let tr = "<tr> <td><input type='checkbox' /></td>" +
                    "<td><select class='netvTabMappingPortTblWlanSelector' style='width:70px'><option value='LTE_4G'>LTE_4G</option><option value='LTE_Z'>LTE_Z</option></select>" +
        ...
        $('#netvTabMappingPortTblApplyBtn').click(()=>{
            ...
            $.ajax({
                ...
                success: (res)=>{
                    ...
                    console.log(data);
                    if(0 == data.rc){
                        oStore.store.Mapping.portMapping = selectedData;
                        // console.log(data.dat);
                        tools.msgBox(tools.jsSwitchLang(enJsMap, cnJsMap, 'operationSucc')+ ' code:' + data.errCode); //'操作成功! 
                    }
                    else{
        ...
        $('#netvTabMappMacIPApplyBtn').click(()=>{
            ...
            $.ajax({
                ...
                success: (res)=>{
                    ...
                    var data = res
                    if(0 == data.rc){
                        oStore.store.Mapping.mac2ip = selectedData;
                        console.log(data.dat);
                        tools.msgBox(tools.jsSwitchLang(enJsMap, cnJsMap, 'operationSucc')+ ' code:' + data.errCode); //'操作成功! 
                    }
                    else{
...
CNetworkView.prototype.enHtmlMap = {
    ...
    netvTabRTMPApplyCfgBtn:"Apply Cfg",RTMPInforTitle:"RTMP Configuration",RTMPInforIPTxt:"Server IP",RTMPInforPortTxt:"Port",RTMPInforAddrTxt:"Push URL",RTMPInforIPValSelectorWarning:"Attention：The selected LAN_IP will be set to 'Static', DHCP will be disbaled on this LAN!",netvTabRTMPNewBtn:"New", netvTabRTMPDelBtn:"Del", netvTabRTMPApplyBtn:"Apply All",netvTabRTMPChannelTableTitle:"RTMP Channel conf.",netvTabRTMPChannelNameTxt:"Channel Name",netvTabRTMPChannelNoTxt:"Channel Code",netvTabRTMPChannelRefTxt:"Full Address",
};
CNetworkView.prototype.cnHtmlMap = {
    ...
    netvTabRTMPApplyCfgBtn:"应用配置修改",RTMPInforTitle:"RTMP Server设置",RTMPInforIPTxt:"Server IP",RTMPInforPortTxt:"端口号",RTMPInforAddrTxt:"推流地址",RTMPInforIPValSelectorWarning:"注意：被选中的IP地址对应的LAN口会被设置为Static, DHCP将在这个LAN失效！",netvTabRTMPNewBtn:"新建", netvTabRTMPDelBtn:"删除", netvTabRTMPApplyBtn:"应用所有",netvTabRTMPChannelTableTitle:"RTMP频道配置表",netvTabRTMPChannelNameTxt:"频道名称",netvTabRTMPChannelNoTxt:"频道编码",netvTabRTMPChannelRefTxt:"频道引用地址",
};
